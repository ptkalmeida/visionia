...
def normalize_text(text):
    ...
    return text.strip()
# Carregar os dados dos ramais
def carregar_ramais():
    caminho = "ramais_extraidos.xlsx"
    ...
    # Esperando colunas como: nome, setor, ramal, email (ajuste aqui se os nomes forem diferentes)
    ramais_data = df.to_dict(orient="records")
    return ramais_data


def buscar_ramal(pergunta):
    ramais = carregar_ramais()
    pergunta_norm = normalize_text(pergunta)

    limiar = 70  # MAIS TOLERANTE
    melhor_match = None
    maior_score = 0

    for ramal in ramais:
        nome = normalize_text(ramal.get("nome", ""))
        setor = normalize_text(ramal.get("setor", ""))

        score_nome = fuzz.partial_ratio(pergunta_norm, nome)
        score_setor = fuzz.partial_ratio(pergunta_norm, setor)

        if score_nome > limiar and score_nome > maior_score:
            melhor_match = ramal
            maior_score = score_nome
        elif score_setor > limiar and score_setor > maior_score:
            melhor_match = ramal
            maior_score = score_setor
...
def get_ai_response(question):
    resultado = buscar_ramal(question)
    if resultado:
        return f"O ramal de {resultado['nome']} é {resultado['ramal']}. Email: {resultado['email']} do setor {resultado['setor']}."

    # Se não for ramal, buscar contexto real nos documentos
    contexto = buscar_contexto(question)
    
    if contexto:
        resposta = perguntar_para_ia(question, contexto, modo="executivo")  # pode usar "resumo", "objetivo", etc
        return resposta.strip()
    else:
        return "Desculpe, não encontrei nenhuma informação nos documentos para responder isso."
...
RESPOSTA OBJETIVA:"""
    ...
    try:
        response = requests.post(
            OLLAMA_URL, 
            json=payload, 
            timeout=300  # Reduzido de 800 para 300 segundos
        )
